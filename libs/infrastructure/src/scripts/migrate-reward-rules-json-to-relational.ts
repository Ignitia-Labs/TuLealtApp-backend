...
/**
 * Script para migrar datos JSON a columnas relacionales en reward_rules
 *
 * Este script ejecuta el SQL de migración de datos que convierte los campos JSON
 * en reward_rules a las nuevas columnas y tablas relacionales.
 *
 * IMPORTANTE:
 * 1. Ejecutar DESPUÉS de la migración AddRelationalColumnsToRewardRules
 * 2. Ejecutar DESPUÉS de crear backups con backup-json-fields.sql
 * 3. Verificar integridad de datos después de ejecutar
 *
 * Uso:
 * npm run script:migrate-reward-rules-json
 */
async function migrateRewardRulesJsonToRelational() {
  ...
  try {
    ...
    try {
      ...
      const sqlPath = path.resolve(
        __dirname,
        '../persistence/migrations/scripts/migrate-reward-rules-json-to-relational.sql',
      );
      ...
      const validationQueries = [
        `SELECT COUNT(*) as total_registros FROM reward_rules`,
        `SELECT COUNT(*) as total_eligibility FROM reward_rule_eligibility`,
        `SELECT COUNT(*) as total_formulas FROM reward_rule_points_formulas`,
        `SELECT COUNT(*) as total_registros, COUNT(CASE WHEN scope_tenant_id = 0 THEN 1 END) as scope_sin_migrar FROM reward_rules`,
      ];
...
if (require.main === module) {
  migrateRewardRulesJsonToRelational()
    .then(() => {
...
export { migrateRewardRulesJsonToRelational };
