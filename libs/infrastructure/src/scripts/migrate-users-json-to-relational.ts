...
/**
 * Script para migrar datos JSON a tablas relacionales en users
 *
 * Este script ejecuta el SQL de migración de datos que convierte los campos JSON
 * en users a las nuevas tablas relacionales.
 *
 * IMPORTANTE:
 * 1. Ejecutar DESPUÉS de la migración AddRelationalColumnsToUsers
 * 2. Ejecutar DESPUÉS de crear backups con backup-json-fields.sql
 * 3. Verificar integridad de datos después de ejecutar
 *
 * Uso:
 * npm run script:migrate-users-json
 */
async function migrateUsersJsonToRelational() {
  ...
  try {
    ...
    try {
      ...
      const sqlPath = path.resolve(
        __dirname,
        '../persistence/migrations/scripts/migrate-users-json-to-relational.sql',
      );
      ...
      const validationQueries = [
        ...
        `SELECT COUNT(*) as total_profile_data FROM user_profile_data`,
        `SELECT COUNT(*) as total_registros, COUNT(CASE WHEN roles IS NOT NULL AND JSON_LENGTH(roles) > 0 THEN 1 END) as usuarios_con_roles_json FROM users`,
        `SELECT COUNT(*) as total_registros, COUNT(CASE WHEN profile IS NOT NULL AND JSON_LENGTH(profile) > 0 THEN 1 END) as usuarios_con_profile_json FROM users`,
      ];
...
export { migrateUsersJsonToRelational };
