...
/**
 * Script para migrar datos JSON a tablas relacionales en customer_tiers
 *
 * Este script ejecuta el SQL de migración de datos que convierte los campos JSON
 * en customer_tiers a las nuevas tablas relacionales.
 *
 * IMPORTANTE:
 * 1. Ejecutar DESPUÉS de la migración AddRelationalColumnsToCustomerTiers
 * 2. Ejecutar DESPUÉS de crear backups con backup-json-fields.sql
 * 3. Verificar integridad de datos después de ejecutar
 *
 * Uso:
 * npm run script:migrate-customer-tiers-json
 */
async function migrateCustomerTiersJsonToRelational() {
  ...
  try {
    ...
    try {
      ...
      const sqlPath = path.resolve(
        __dirname,
        '../persistence/migrations/scripts/migrate-customer-tiers-json-to-relational.sql',
      );
      ...
      const validationQueries = [
        `SELECT COUNT(*) as total_registros FROM customer_tiers`,
        `SELECT COUNT(*) as total_tier_benefits FROM customer_tier_benefits`,
        `SELECT COUNT(*) as total_registros, COUNT(CASE WHEN benefits IS NOT NULL AND JSON_LENGTH(benefits) > 0 THEN 1 END) as tiers_con_benefits_json FROM customer_tiers`,
      ];
...
if (require.main === module) {
  migrateCustomerTiersJsonToRelational()
    .then(() => {
...
export { migrateCustomerTiersJsonToRelational };
