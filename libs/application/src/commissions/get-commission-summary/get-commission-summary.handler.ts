...
export class GetCommissionSummaryHandler {
  ...
  async execute(request: GetCommissionSummaryRequest): Promise<GetCommissionSummaryResponse> {
    // Verificar que el usuario staff existe
    const staffUser = await this.userRepository.findById(request.staffUserId);

    if (!staffUser) {
      throw new NotFoundException(`Staff user with ID ${request.staffUserId} not found`);
    }
    ...
    const endDate = request.endDate ? new Date(request.endDate) : new Date(); // Hoy
