...
import { IPartnerStaffAssignmentRepository } from '@libs/domain';
...
export class DeletePartnerStaffAssignmentHandler {
  ...
  async execute(request: DeletePartnerStaffAssignmentRequest): Promise<void> {
    // Verificar que la asignación existe
    const assignment = await this.assignmentRepository.findById(request.id);

    if (!assignment) {
      throw new NotFoundException(`Partner staff assignment with ID ${request.id} not found`);
    }
