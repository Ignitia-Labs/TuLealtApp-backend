...
export const BASE64_IMAGE_ALLOWED_MIMES = ['image/jpeg', 'image/jpg', 'image/png', 'image/webp'];
...
export function validateBase64Image(value: string): { buffer: Buffer; mimetype: string } {
  ...
  if (!BASE64_IMAGE_ALLOWED_MIMES.includes(mimetype)) {
    throw new BadRequestException('Invalid image format. Only png, jpg, jpeg and webp are allowed');
  }
